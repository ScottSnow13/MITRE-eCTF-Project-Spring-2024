...
// Datatype for recording components keys and addresses
typedef struct {
	uint8_t address;
	uint8_t compkey[KEYSIZE];
} comp_table;

/********************************* GLOBAL VARIABLES **********************************/
// Variable for information stored in flash memory
flash_entry flash_status;
int keysize = KEYSIZE; //change to size of the key in bits
uint8_t symkey[KEYSIZE];
uint8_t publicDERkey[KEYSIZE];
uint8_t privDERkey[KEYSIZE];
uint8_t * componentkeys[COMPONENT_CNT];
comp_table keytable[COMPONENT_CNT];
int keysStored = 0;
...
int secure_send(uint8_t address, uint8_t* buffer, uint8_t len) {
	...
	for(int i = 0 ; i < keysStored ; i++){
		if(strcmp(&(keytable[i].address),&address)) {
			memcpy(ckey, keytable[i].compkey, keysize);
		}
	}
	RsaKey * pkey;
    pkey = (RsaKey*)malloc(sizeof(RsaKey));
	wc_RsaPublicKeyDecode(ckey, ckey, pkey, keysize);
	int error2 = wc_RsaPublicEncrypt(encryptedtext, len, encryptedtext2, len, pkey, rng);
    free(pkey);
    return send_packet(address, len, encryptedtext2);
}
...
int secure_receive(i2c_addr_t address, uint8_t* buffer) {
    int bytesreceived = poll_and_receive_packet(address, buffer);
    uint8_t ciphertext[bytesreceived];
    memcpy(ciphertext, buffer, bytesreceived);
...
int validate_components() {
    ...
    for (unsigned i = 0; i < flash_status.component_cnt; i++) {
        ...
        memcpy(command->params, publicDERkey, KEYSIZE);
		...
		keytable[i].address = addr;
		memcpy(keytable[i].compkey, validate->pubkey, KEYSIZE);
		componentkeys[i] = malloc(sizeof(uint8_t) * KEYSIZE);
		memcpy(componentkeys[i], validate->pubkey, KEYSIZE);
		keysStored++;
		
	uint8_t *check_key;
	hash(COMP_SECRET, 32, check_key);
	if (memcmp(check_key, validate->key, 32) != 0) {
	    print_error("Component ID: 0x%08x invalid\n", flash_status.component_ids[i]);
	    return ERROR_RETURN;
        }
	}
...
int boot_components() {
    ...
    for (unsigned i = 0; i < flash_status.component_cnt; i++) {
		...
		uint8_t check_ap[32];
        hash(AP_SECRET, sizeof(AP_SECRET), check_ap);
		memcpy(command->params, check_ap, sizeof(check_ap));
...
int main() {
	...
	if (wc_MakeRsaKey(&key, keysize, exponent, rng)) {
		// Handle error
	...
	//encrypt pin and token
	wc_Sha256Hash(AP_PIN, 6, pin);
	wc_Sha256Hash(AP_TOKEN, 16, token);
	
	wc_Sha256Hash(AP_SECRET,keysize * 8,symkey);
